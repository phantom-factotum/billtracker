...
export default function Home({navigation}){
  ...
  const tutorialTextStyle = useAnimatedStyle(()=>{
    return {
			opacity:interpolate(addExpensesTask.anim.value,[0, 0.5 , 1],[0, 0.0, 1])
		}
  })
	...
	return (
		<View style={[styles.container,{height:isLandScape ? '100%': '95%'}]}>
			{expenses.length < 5 && (
        <Animated.View style={{paddingTop:headerHeight*1.5,width:2,alignItems:'center',...tutorialButtonAnimation}}>
          <TouchableOpacity 
          ...
          {addExpensesTask.started && 
						<View style={{alignItems:'center'}}>
							<Animated.Text style={tutorialTextStyle}> Press Me</Animated.Text>
						</View>
          }
        </Animated.View>
			)}
