...
// logs a color square and the hex for the color
const logColor = color=>{
	if(myChalk.level < 0)
		return
	console.log(myChalk.bgHex(color)('  '),color)
}
// logs a list color squares attempting 20 per line
const logColorSquares = colors=>{
	if(myChalk.level < 0)
		return
	let squares = colors.map(color=>myChalk.bgHex(color)('  '))
	let str = ''
...
// 3 types of color generators

// generates colors using golden ratio to get equal distance
...
// grabs a random color, generate a scheme for it,remove similar colors, 
// and repeats until it has reached desire length
export const genColorsWithScheme=len=>{
	// max attempt to generate colors before changing lowering threshold (color distance)
	let thresholdLevelAttempts = Math.max(len/4,15);
	...
	while(colors.length < len){
		if(attempts > thresholdLevelAttempts){
			...
			thresholdLevelAttempts = Math.round(thresholdLevelAttempts*1.5)
		}
		// get random color and generate schemes from that color
...
// the color generator object itself
